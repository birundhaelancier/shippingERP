...
import {
  ...
  EditCustomerAddress,
  getCustomerList,
} from "../../../Redux/Action/GeneralGroupAction/customerAction";
...
export default function AddressInfo({
  ...
  const dynObjs = {
    ...
    state: { value: "", validation: [], error: null, errmsg: null },
    phone2: {
      value: "",
      validation: [{ name: "required" }, { name: "mobilenumber" }],
      error: null,
      errmsg: null,
    },
    city: {
  ...
  const onSubmit = () => {
    ...
    Itemkeys.map((item, index) => {
      if (Nommiee[item][targetkeys[index]].value !== "") {
        filtererr = targetkeys.filter(
          (obj) => BasicInformation[obj].error == null
        );
      } else {
        filtererr = targetkeys.filter(
          (obj) => BasicInformation[obj].error == true
        );
      }
    });
    ...
    if (filtererr.length > 0) {
      ...
      if (customerId) {
        EditCustomerAddress(header, customerId).then((res) => {
        ...
        AddCustomerAddress(header, userId).then((res) => {
    ...
    <div>
                  ...
                  <Grid container spacing={2}>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        changeData={(data) =>
                          OnChangeNommiee(data, "address_type", item, index)
                        }
                        dropdown={addressType}
                        value={
                          Nommiee[item]["address_type"].value == ""
                            ? BasicInformation.address_type.value
                            : Nommiee[item]["address_type"].value
                        }
                        error={
                          Nommiee[item]["address_type"].value == ""
                            ? BasicInformation.address_type.error
                            : Nommiee[item]["address_type"].error
                        }
                        errmsg={
                          Nommiee[item]["address_type"].value == ""
                            ? BasicInformation.address_type.errmsg
                            : Nommiee[item]["address_type"].errmsg
                        }
                      />
                    </Grid>
                    ...
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="Address1"
                        changeData={(data) =>
                          OnChangeNommiee(data, "address1", item, index)
                        }
                        value={
                          Nommiee[item]["address1"].value == ""
                            ? BasicInformation.address1.value
                            : Nommiee[item]["address1"].value
                        }
                        error={
                          Nommiee[item]["address1"].value == ""
                            ? BasicInformation.address1.error
                            : Nommiee[item]["address1"].error
                        }
                        errmsg={
                          Nommiee[item]["address1"].value == ""
                            ? BasicInformation.address1.errmsg
                            : Nommiee[item]["address1"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="Address2"
                        changeData={(data) =>
                          OnChangeNommiee(data, "address2", item, index)
                        }
                        value={
                          Nommiee[item]["address2"].value == ""
                            ? BasicInformation.address2.value
                            : Nommiee[item]["address2"].value
                        }
                        error={
                          Nommiee[item]["address2"].value == ""
                            ? BasicInformation.address2.error
                            : Nommiee[item]["address2"].error
                        }
                        errmsg={
                          Nommiee[item]["address2"].value == ""
                            ? BasicInformation.address2.errmsg
                            : Nommiee[item]["address2"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="Country"
                        changeData={(data) =>
                          OnChangeNommiee(data, "country", item, index)
                        }
                        dropdown={CountryList}
                        value={
                          Nommiee[item]["country"].value == ""
                            ? BasicInformation.country.value
                            : Nommiee[item]["country"].value
                        }
                        error={
                          Nommiee[item]["country"].value == ""
                            ? BasicInformation.country.error
                            : Nommiee[item]["country"].error
                        }
                        errmsg={
                          Nommiee[item]["country"].value == ""
                            ? BasicInformation.country.errmsg
                            : Nommiee[item]["country"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="State"
                        changeData={(data) =>
                          OnChangeNommiee(data, "state", item, index)
                        }
                        dropdown={StateList}
                        value={
                          Nommiee[item]["state"].value == ""
                            ? BasicInformation.state.value
                            : Nommiee[item]["state"].value
                        }
                        error={
                          Nommiee[item]["state"].value == ""
                            ? BasicInformation.state.error
                            : Nommiee[item]["state"].error
                        }
                        errmsg={
                          Nommiee[item]["state"].value == ""
                            ? BasicInformation.state.errmsg
                            : Nommiee[item]["state"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="City"
                        changeData={(data) =>
                          OnChangeNommiee(data, "city", item, index)
                        }
                        dropdown={CityList}
                        value={
                          Nommiee[item]["city"].value == ""
                            ? BasicInformation.city.value
                            : Nommiee[item]["city"].value
                        }
                        error={
                          Nommiee[item]["city"].value == ""
                            ? BasicInformation.city.error
                            : Nommiee[item]["city"].error
                        }
                        errmsg={
                          Nommiee[item]["city"].value == ""
                            ? BasicInformation.city.errmsg
                            : Nommiee[item]["city"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        showFlag
                        changeData={(data) =>
                          OnChangeNommiee(data, "phone2", item, index)
                        }
                        value={
                          Nommiee[item]["phone2"].value == ""
                            ? BasicInformation.phone2.value
                            : Nommiee[item]["phone2"].value
                        }
                        error={
                          Nommiee[item]["phone2"].value == ""
                            ? BasicInformation.phone2.error
                            : Nommiee[item]["phone2"].error
                        }
                        errmsg={
                          Nommiee[item]["phone2"].value == ""
                            ? BasicInformation.phone2.errmsg
                            : Nommiee[item]["phone2"].errmsg
                        }
                      />
                    </Grid>
                    ...
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="Zip Code"
                        changeData={(data) =>
                          OnChangeNommiee(data, "zip_code", item, index)
                        }
                        value={
                          Nommiee[item]["zip_code"].value == ""
                            ? BasicInformation.zip_code.value
                            : Nommiee[item]["zip_code"].value
                        }
                        error={
                          Nommiee[item]["zip_code"].value == ""
                            ? BasicInformation.zip_code.error
                            : Nommiee[item]["zip_code"].error
                        }
                        errmsg={
                          Nommiee[item]["zip_code"].value == ""
                            ? BasicInformation.zip_code.errmsg
                            : Nommiee[item]["zip_code"].errmsg
                        }
                      />
                    </Grid>
                    <Grid item md={4} xs={12} lg={4}>
                      <Labelbox
                        ...
                        labelname="Fax"
                        changeData={(data) =>
                          OnChangeNommiee(data, "fax", item, index)
                        }
                        value={
                          Nommiee[item]["fax"].value == ""
                            ? BasicInformation.fax.value
                            : Nommiee[item]["fax"].value
                        }
                        error={
                          Nommiee[item]["fax"].value == null
                            ? BasicInformation.fax.error
                            : Nommiee[item]["fax"].error
                        }
                        errmsg={
                          Nommiee[item]["fax"].value == ""
                            ? BasicInformation.fax.errmsg
                            : Nommiee[item]["fax"].errmsg
                        }
                      />
                    </Grid>
                    {/* <Grid item md={4} xs={12} lg={4}>
      ...
      >
        <FooterBtn
          ...
          saveBtn={"Save Stage"}
          onSaveBtn={onSubmit}
          onBack={() => handleActivekey("0")}
          nextDisable={
            ViewCustomer[0] && ViewCustomer[0]?.address.length > 0
              ? false
              : true
          }
        />
