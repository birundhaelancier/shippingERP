...
import { AddCustomerKyc, ViewCustomerDetails, EditCustomerKyc, getCustomerList } from '../../../Redux/Action/GeneralGroupAction/customerAction';


export default function KycDeatils({ handleActivekey, customerId, userId }) {
    ...
    const onSubmit = () => {
        ...
        if (filtererr.length > 0) {
            ...
            if (customerId) {
                ...
                AddCustomerKyc(kycInfo, header, userId)
                    .then((res) => {
                        if (res?.Status === 'Success') {
                            notification.success({
                                message: res?.Message
