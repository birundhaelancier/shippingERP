...
export default function GeneralInfo({ shipTermId, shipTermName }) {
  ...
  useEffect(() => {
    dispatch(ShipmentTermList(1));
  }, [shipTermId]);

  useEffect(() => {
    if (shipTermId) {
      CargoInfo.CargoName.value = shipTermName;
      setCargoInfo((prevState) => ({
        ...prevState,
      }));
    }
  }, [shipTermId, shipTermName]);
  ...
  const onSubmit = () => {
    ...
    if (filtererr.length > 0) {
      ...
      if (shipTermId) {
        dispatch(EditShipmentTerm(CargoInfo, shipTermId)).then(() => {
          history.push("/shipmentTerm");
          //   HandleCancel();
        });
      } else {
        dispatch(AddShipmentTerm(CargoInfo)).then(() => {
          history.push("/shipmentTerm");
          //   HandleCancel();
        });
      }
