...
import { CargoList,CargoStatus,DeleteCargoList } from '../../Redux/Action/CargoAction'
      ...
      },
        {
            ...
            renderCell: (params) => {
                return (
                    ...
                    <Link to={`/addcargo/${params.row.portId}`} className="editSymbol" ><Edit /></Link>
    ...
    useEffect(()=>{
        ...
        GetCargoList?.map((items,index)=>{
            rows.push(
                {
                    id: index+1,
                    cargoId: items.section_name,
                    cargoName: items.chapter_name,
