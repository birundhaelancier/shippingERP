...
import { AddVendorDocument, ViewVendorDetails, EditVendorDocument, getVendorList } from '../../../Redux/Action/GeneralGroupAction/VendorAction';

export default function Documents({ handleActivekey, vendorId, userId }) {
    const Description = [
    ...
    const onSubmit = () => {
        ...
        if (filtererr.length > 0) {
            ...
            if (vendorId) {
                ...
                AddVendorDocument(header, userId)
                    .then((res) => {
                        if (res?.Status === 'Success') {
                            notification.success({
                                message: res?.Message
